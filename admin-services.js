...
async function loadServices() {
  ...
  // Compte le nombre de demandes reçues par service (table commandes)
  const orderCounts = {};
  const { data: ordersData } = await supabaseClient.from('commandes').select('service_id');
  (ordersData || []).forEach(o => {
    if (o.service_id) orderCounts[o.service_id] = (orderCounts[o.service_id] || 0) + 1;
  });
 
  grid.innerHTML = currentServices.map(s => {
    const count = orderCounts[s.id] || 0;
    return `
    <div class="admin-svc-card">
      ...
      <div class="admin-svc-body">
        ...
        <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
          ...
          <span class="pill pill-nouveau">${count} demande${count > 1 ? 's' : ''}</span>
        </div>
